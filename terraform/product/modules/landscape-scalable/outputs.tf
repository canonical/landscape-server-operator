...
output "applications" {
  ...
  value = {
    ...
    haproxy_certs    = var.haproxy_certs != null && length(juju_application.haproxy_certs) > 0 ? juju_application.haproxy_certs[0] : null
...
locals {
  haproxy_self_signed = var.haproxy != null && length(juju_application.haproxy_certs) > 0
}
