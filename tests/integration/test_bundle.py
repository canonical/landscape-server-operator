...
def test_smtp_relation(juju: jubilant.Juju, bundle: None):
    """
    When smtp-integrator is related to landscape-server, postfix is configured
    with the relay host and SASL credentials.
    """
    juju.wait(jubilant.all_active, timeout=300)
    ...
    juju.integrate("landscape-server:smtp", "smtp-integrator")
    ...
    try:
        ...
        assert "smtp.example.com" in relayhost, (
            f"Expected smtp.example.com in relayhost, got: {relayhost}"
        )

        sasl_passwd = juju.ssh(
            unit_name,
            "sudo cat /etc/postfix/sasl_passwd",
        ).strip()
        assert "smtp.example.com" in sasl_passwd
        assert "landscape" in sasl_passwd
    finally:
        juju.remove_relation("landscape-server:smtp", "smtp-integrator")
        juju.remove_application("smtp-integrator")
        juju.wait(jubilant.all_active, timeout=300)
        juju.wait(jubilant.all_active, timeout=300)
